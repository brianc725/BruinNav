...
int main()
{
    ...
    {
        MapLoader ml;
        assert(ml.load("/Users/brianchan/Desktop/Project4/Project4/testmap.txt"));
    ...
    assert(ml.load("/Users/brianchan/Desktop/Project4/Project4/testmap.txt"));
    Navigator nm;
    assert(nm.loadMapData("/Users/brianchan/Desktop/Project4/Project4/testmap.txt"));
    vector <NavSegment> dir;
    cout << nm.navigate("Eros Statue", "Hamleys Toy Store", dir) << endl;
    for (int i = 0; i < dir.size(); i++)
    {
        cout << dir[i].m_streetName << endl;
        cout << dir[i].m_distance << endl;
    }
    
    MyMap<string, double> nameToGPA;
    ...
    if (davidsGPA == nullptr)
        cout << "David is not in the roster!" << endl;
    else
        cout << "David's GPA is: " << *davidsGPA << endl;
    ...
    MapLoader m;
    m.load("/Users/brianchan/Desktop/Project4/Project4/mapdata.txt");
    //m.load("/Users/brianchan/Desktop/Project4/Project4/mapsmall.txt");
    
    
    AttractionMapper am;
    am.init(m); // let our object build its internal data structures
    // by iterating thru all segments from the MapLoader object
    GeoCoord fillMe;
    string attraction = "The Coffee Bean & Tea Leaf_2";
    ...
    attraction = "ThE CoFFee BeaN & TEA LeAf_2";
    found = am.getGeoCoord(attraction, fillMe);
    if ( ! found)
    {
        cout << "No geolocation found for " << attraction << endl;
    }
    ...
    SegmentMapper sm;
    sm.init(m); // let our object build its internal data structures
    // by iterating thru all segments from the MapLoader object
    GeoCoord lookMeUp(fillMe);
    ...
    GeoCoord lookUp("34.0600264", "-118.4460993");
    std::vector<StreetSegment> vecOfAssSegs(sm.getSegments(lookUp));
    if (vecOfAssSegs.empty())
    {
        cout << "Error - no segments found matching this coordinate\n";
    }
    cout << "Here are all the segments associated with your coordinate:" << endl;
    for (auto s: vecOfAssSegs)
    {
        cout << "Segment’s street: " << s.streetName << endl;
        cout << "Segment’s start lat/long: " << s.segment.start.latitudeText << ", " <<
        s.segment.start.longitudeText << endl;
    ...
    GeoCoord lookMeUp2("34.0619693", "-118.4479670");
    std::vector<StreetSegment> vecOfAssociatedSegs2(sm.getSegments(lookMeUp2));
    if (vecOfAssociatedSegs2.empty())
    {
        cout << "Error - no segments found matching this coordinate\n";
    }
    cout << "Here are all the segments associated with your coordinate:" << endl;
    for (auto s: vecOfAssociatedSegs2)
    {
        cout << "Segment’s street: " << s.streetName << endl;
        cout << "Segment’s start lat/long: " << s.segment.start.latitudeText << ", " <<
        s.segment.start.longitudeText << endl;
        ...
        switch (ns.m_command)
        {
            ...
            case NavSegment::TURN:
                if (distSinceLastTurn > 0)
                {
                    ...
                    << getAngle(effectiveSegment) << " on " << thisStreet << endl;
                    thisStreet.clear();
                    distSinceLastTurn = 0;
                }
